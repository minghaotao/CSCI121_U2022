...
int main() {
    ...
    int day, month, year;
    do {
//        system("CLS"); this is for windows command
...
//                print_calendar(month, year);//print out the calendar of specified month
                ofstream out;
                out.open("edward_tao.txt");
                print_calendar(month, year, out);
                out.close();
                break;
            }
            ...
            case '4': // print out the calendar of specified year to yourname.txt file
                // you implement this ******************
                get_input(year);
                print_calendar(year, "edwardtao.txt");
                cout << "Calednar of " << year << " has been printed out to edwardtao.txt" << endl;
                break;
        ...
        cout << "\n\n\t";
//        system("pause"); this is for windows
...
void menu() {
    ...
    cout << "\n\t* 4. Print calendar of a give year to file        *";
...
//        exit(1);
    ...
    for (int i = 1; i <= 12; i++) {
        outstream << "\n\t " << monthName(i) << ", " << year << endl;
        outstream << "\n\t SUN MON TUE WED THU FRI SAT\n\t";
        int spaceCount = dayOfWeek(i, 1, year);
        // print out spaces
        for (int s = 0; s < spaceCount; s++)
            outstream << setw(4) << "";
        // calculate the days in the month
        int days = daysInMonth(i, year);
        // print out all days
        for (int j = 1; j <= days; j++) {
            outstream << setw(4) << j;
            if ((spaceCount + j) % 7 == 0) // go to new line for a new week
